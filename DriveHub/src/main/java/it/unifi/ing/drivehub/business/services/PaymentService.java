...
public final class PaymentService {
    ...
    public Payment payRental(long customerId, long rentalId, PaymentMethod method) {
        return transactions.execute(unit -> {
            ...
            Rental rental = unit.rentals().findById(rentalId)
    ...
    public List<Payment> paymentsFor(PaymentReferenceType referenceType, long referenceId) {
        return transactions.execute(unit -> List.copyOf(
                unit.payments().findByReference(referenceType, referenceId)));
    }
    ...
    private static void validateSalePayment(SaleOrder order, PaymentPurpose purpose, BigDecimal amount) {
        ...
        if (purpose == PaymentPurpose.SALE_DEPOSIT) {
            ...
            if (amount.compareTo(depositOutstanding) < 0) {
                throw new ConflictException("payment does not cover the required deposit");
            }
        } else {
            if (order.status() != SaleOrderStatus.DEPOSIT_PAID) {
                throw new ConflictException("balance is accepted only after the deposit");
            }
            if (amount.compareTo(outstanding) != 0) {
                throw new ConflictException("balance payment must cover the exact outstanding amount");
            }
        }
    }
